...
namespace Basit_Ajanda
{
    public partial class Not_Ekle : Form
    {
        ...
        void not_oku()
        {
            ...
            {
                DataTable dt = Cls.SQLConnectionClass.Table("select not_RECno, not_Baslik, not_Icerik from Table_NOTLAR1  where not_RECno = " + Convert.ToInt32(txt_kayit_no.Text) + " ");
        ...
        private void btn_kaydet_Click(object sender, EventArgs e)
        {
            if (Convert.ToInt32(txt_kayit_no.Text) == 0)
            {
                Cls.SQLConnectionClass.Command(""
                          ...
                          + "             ,'" + Convert.ToDateTime(dateTimePicker2.Value).ToString("  yyyyMMdd HH:mm") + "'"
                          + "             ,'" + txt_icerik.Text + "') "
            ...
            else
            {
                ...
                Cls.SQLConnectionClass.Command(""
                      ...
                      + "             ,[not_BitisTarih] = '" + Convert.ToDateTime(dateTimePicker2.Value).ToString("    yyyyMMdd HH:mm") + "' "
        ...
        private void btn_sil_Click(object sender, EventArgs e)
        {
            ...
            Cls.SQLConnectionClass.Command("delete from [dbo].[Table_NOTLAR1]    where not_RECno = " + Convert.ToInt32(txt_kayit_no.Text) + " ");
            if (Cls.SQLConnectionClass.exception == null)
            {
